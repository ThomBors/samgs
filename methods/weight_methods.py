...
class AlignedMTL(WeightMethod):
    ...
    def __init__(self, n_tasks, device: torch.device, scale_mode='min', max_norm=1.0):
        super().__init__(n_tasks, device=device)
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            ...
            for p in shared_parameters:
                p.grad = None
        g,aligned_grads, weights = self.alignedmtl(grads, scale_mode=self.scale_mode, rescale=1)
        self.overwrite_grad(shared_parameters, g, grad_dims)
    ...
    def backward(
        ...
        **kwargs,
        ...
        g,aligned_grads, w = self.get_weighted_loss(losses, shared_parameters)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
...
class  SAMGS(WeightMethod):
    """Similarity Aware Momentum Gradient Surgery"""
    ...
    def balance_magnitude(self,grads):
        ...
        self.v = self.b2*self.v + ((1-self.b2)*(1-similarities.mean())**2)+1e-8
